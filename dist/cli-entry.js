...
async function main() {
    ...
    if (cmd === 'migrate') {
        ...
        await runMigrate(getSquadStartDir(), { to, from: from, dryRun });
        return;
    }
    ...
    if (cmd === 'triage' || cmd === 'watch') {
        ...
        const rawDispatchMode = (dispatchModeIdx !== -1 && args[dispatchModeIdx + 1])
            ...
            : undefined;
        const validModes = ['task', 'fleet', 'hybrid'];
        const dispatchMode = rawDispatchMode && validModes.includes(rawDispatchMode)
            ? rawDispatchMode
            : rawDispatchMode
        ...
        const rawNotifyLevel = (notifyLevelIdx !== -1 && args[notifyLevelIdx + 1])
            ...
            : undefined;
        const validNotifyLevels = ['all', 'important', 'none'];
        const notifyLevel = rawNotifyLevel && validNotifyLevels.includes(rawNotifyLevel)
            ? rawNotifyLevel
            : rawNotifyLevel
        ...
        if (rawStateBackend && !validBackends.includes(rawStateBackend)) {
            console.error(`\u26a0\ufe0f Invalid --state-backend "${rawStateBackend}". Valid: ${validBackends.join(', ')}.`);
            process.exit(1);
        }
        const mappedBackend = rawStateBackend;
        ...
        const stateContext = resolveSquadState(getSquadStartDir(), mappedBackend);
        // Build capability overrides from CLI flags and --no-{cap} flags
        const capabilities = {};
        const registry = createDefaultRegistry();
        for (const cap of registry.all()) {
            if (args.includes(`--${cap.name}`))
                capabilities[cap.name] = true;
            if (args.includes(`--no-${cap.name}`))
