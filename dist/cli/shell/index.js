...
import { RateLimitError } from '@deepakkamboj/squad-sdk/adapter/errors';
import { FSStorageProvider, initSquadTelemetry, TIMEOUTS, StreamingPipeline, recordAgentSpawn, recordAgentDuration, recordAgentError, recordAgentDestroy, RuntimeEventBus, resolveSquad, resolveGlobalSquadPath } from '@deepakkamboj/squad-sdk';
...
export async function runShell() {
    ...
    async function handleInitCast(parsed, skipConfirmation) {
        ...
        try {
            ...
            if (useBaseRoles) {
                try {
                    await storage.delete(initRolesMarker);
                }
                catch { /* ignore */ }
            }
    ...
    async function finalizeCast(proposal, parsed) {
        ...
        if (!hasRosterEntries(verifyContent)) {
            ...
            if (storage.existsSync(initPromptCleanup)) {
                try {
                    await storage.delete(initPromptCleanup);
                }
                catch { /* ignore */ }
            }
            shellApi?.addMessage({
        ...
        if (storage.existsSync(initPromptFile)) {
            try {
                await storage.delete(initPromptFile);
            }
            catch { /* ignore */ }
        }
            ...
            if (shellApi) {
                ...
                if (isRateLimit) {
                    ...
                    try {
                        ...
                        storage.writeSync(join(squadDir, 'rate-limit-status.json'), JSON.stringify({
                            timestamp: new Date().toISOString(),
                            retryAfter,
                            model,
                            message: errorMsg,
        ...
        onReady: (api) => {
            ...
            if (storage.existsSync(teamFilePath)) {
                ...
                if (hasRosterEntries(tc) && storage.existsSync(initPromptPath)) {
                    ...
                    void storage.delete(initPromptPath).catch(() => { });
                }
            }
