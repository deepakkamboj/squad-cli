...
import { createCompleter, getSuggestions } from '../autocomplete.js';
...
const SuggestionBox = ({ suggestions, selectedIndex, accent, noColor, terminalWidth }) => {
    ...
    return (_jsxs(Box, { flexDirection: "column", children: [visible.map((s, i) => {
                ...
                const dim = !selected;
                return (_jsxs(Box, { children: [_jsxs(Text, { color: color, dimColor: dim, bold: selected, children: [selected ? ' › ' : '   ', label] }), desc ? _jsx(Text, { dimColor: true, children: desc }) : null] }, s.label));
            }), overflow > 0 && (_jsxs(Text, { dimColor: true, children: ["   \u2026 ", overflow, " more \u2014 keep typing to filter"] }))] }));
};
export const InputPrompt = ({ onSubmit, prompt = '> ', disabled = false, agents = [], messageCount = 0, }) => {
    ...
    const suggestions = useMemo(() => (disabled ? [] : getSuggestions(value, agents)), [value, agents, disabled]);
    ...
    if (disabled) {
        return (_jsxs(Box, { flexDirection: "column", children: [_jsx(Box, { children: noColor ? (_jsxs(_Fragment, { children: [_jsx(Text, { bold: true, children: narrow ? brand.narrowPrompt : brand.prompt }), _jsx(Text, { children: "[working...]" }), bufferDisplay ? _jsxs(Text, { children: [" ", bufferDisplay] }) : null] })) : (_jsxs(_Fragment, { children: [_jsx(Text, { color: accent, bold: true, children: narrow ? brand.narrowPrompt : brand.prompt }), _jsx(Text, { color: accent, children: SPINNER_FRAMES[spinFrame] }), bufferDisplay ? _jsx(Text, { dimColor: true, children: bufferDisplay }) : null] })) }), !bufferDisplay && _jsx(Text, { dimColor: true, children: "[working...]" })] }));
    }
    return (_jsxs(Box, { flexDirection: "column", children: [showSuggestions && (_jsx(SuggestionBox, { suggestions: suggestions, selectedIndex: suggestionIndex, accent: noColor ? '' : accent, noColor: noColor, terminalWidth: width })), _jsxs(Box, { children: [_jsx(Text, { color: noColor ? undefined : accent, bold: true, children: narrow ? brand.narrowPrompt : brand.prompt }), _jsx(Text, { children: value }), _jsx(Text, { color: noColor ? undefined : accent, bold: true, children: "\u258C" })] }), !value && !showSuggestions && (_jsx(Text, { dimColor: true, children: getHintText(messageCount, narrow) })), showSuggestions && (_jsx(Text, { dimColor: true, children: "  \u2191\u2193 navigate \u00B7 Tab/Enter select \u00B7 Esc dismiss" }))] }));
};
