...
function dirSize(dir) {
    ...
    for (const f of files) {
        try {
            bytes += storage.statSync(f)?.size ?? 0;
        }
        catch { /* skip */ }
    }
...
function fileSize(p) {
    try {
        return storage.statSync(p)?.size ?? 0;
    }
    catch {
        return 0;
    }
...
function collectMetrics(squadDir) {
    ...
    if (storage.existsSync(agentsDir)) {
        for (const name of storage.listSync(agentsDir)) {
            if (!storage.isDirectorySync(path.join(agentsDir, name)))
                continue;
...
function archiveDecisions(squadDir, dryRun) {
    const decisionsFile = path.join(squadDir, 'decisions.md');
    if (!storage.existsSync(decisionsFile))
        return null;
...
function writeJournal(squadDir) {
    storage.writeSync(path.join(squadDir, JOURNAL_FILE), `nap started at ${new Date().toISOString()}\n`);
}
function removeJournal(squadDir) {
    const jp = path.join(squadDir, JOURNAL_FILE);
    if (storage.existsSync(jp))
        storage.deleteSync(jp);
}
// ─── Main entry ─────────────────────────────────────────────────────────
