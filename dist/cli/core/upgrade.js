...
function detectProjectType(dir) {
    if (storage.existsSync(path.join(dir, 'package.json')))
        return 'npm';
    if (storage.existsSync(path.join(dir, 'go.mod')))
        return 'go';
    if (storage.existsSync(path.join(dir, 'requirements.txt')) ||
        storage.existsSync(path.join(dir, 'pyproject.toml')))
        return 'python';
    if (storage.existsSync(path.join(dir, 'pom.xml')) ||
        storage.existsSync(path.join(dir, 'build.gradle')) ||
        storage.existsSync(path.join(dir, 'build.gradle.kts')))
        return 'java';
    try {
 ...
 */
export function ensureCastingDefaults(dest, templatesDir) {
 ...
 */
function warnIfSkillCustomized(srcPath, destPath, sourceName) {
    if (!storage.existsSync(destPath))
        return;
...
function syncAllSkills(dest, templatesDir) {
    ...
    for (const entry of skillEntries) {
        ...
        if (!storage.existsSync(srcPath))
            continue;
        if (!entry.overwriteOnUpgrade && storage.existsSync(destPath))
            continue;
 ...
 */
export async function selfUpgradeCli(options = {}) {
