...
import { GREEN, YELLOW, BOLD, DIM, RESET } from '../../core/output.js';
 ...
 */
export async function loadExternalCapabilities(teamRoot, registry) {
    ...
    console.log(`${YELLOW}    External capabilities execute with FULL process permissions (file system, network, credentials).${RESET}`);
    console.log(`${YELLOW}    Review these files if you did not author them.${RESET}\n`);
    let loaded = 0;
    for (const filename of jsFiles) {
        const filePath = join(capDir, filename);
        try {
            const mod = await import(pathToFileURL(filePath).href);
            let cap = mod.default ?? mod;
            ...
            if (missing.length > 0) {
                console.log(`${YELLOW}⚠️ Failed to load capability from ${filename}: missing fields: ${missing.join(', ')}${RESET}`);
                continue;
            }
            // Validate field types
            if (typeof cap.preflight !== 'function' || typeof cap.execute !== 'function') {
                console.log(`${YELLOW}⚠️ Failed to load capability from ${filename}: preflight and execute must be functions${RESET}`);
                continue;
            }
            const validPhases = ['pre-scan', 'post-triage', 'post-execute', 'housekeeping'];
            if (!validPhases.includes(cap.phase)) {
                console.log(`${YELLOW}⚠️ Failed to load capability from ${filename}: invalid phase '${cap.phase}' (must be one of: ${validPhases.join(', ')})${RESET}`);
                continue;
            }
            // Security: block hijacking of built-in capabilities
            if (registry.get(cap.name)) {
                console.log(`${YELLOW}⚠️ External capability "${cap.name}" conflicts with built-in — skipped (potential hijack)${RESET}`);
                continue;
            }
            registry.register(cap);
            console.log(`${GREEN}✅ Loaded external capability: ${cap.name} (phase: ${cap.phase})${RESET}`);
            loaded++;
        }
        catch (err) {
            console.log(`${YELLOW}⚠️ Failed to load capability from ${filename}: ${err instanceof Error ? err.message : String(err)}${RESET}`);
        }
    }
    return loaded;
}
