...
export async function delegateCommand(args) {
    ...
    if (!target) {
        ...
        fatal(`Squad "${squadName}" not found.` +
    ...
    try {
        ...
        fatal(`Failed to create issue: ${err.message}`);
    }
}
function buildDelegationBody(description, target) {
    return [
