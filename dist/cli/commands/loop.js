...
export function generateLoopFile() {
...
function buildLoopAgentCommand(prompt, options) {
    if (options.agentCmd) {
...
async function runPhase(phase, enabled, context, config) {
    ...
    for (const cap of phaseCapabilities) {
        try {
            ...
            console.log(`${YELLOW}⚠${RESET} [${ts}] ${cap.name} crashed: ${e.message}`);
        }
    }
}
// ── Preflight Capabilities ───────────────────────────────────────
async function preflightLoopCapabilities(registry, config, context) {
    const enabled = [];
    const skipped = [];
    // Loop only activates pre-scan and housekeeping phases
    const allowedPhases = ['pre-scan', 'housekeeping'];
    for (const cap of registry.all()) {
        if (!allowedPhases.includes(cap.phase))
            continue;
        const capConfig = config.capabilities[cap.name];
        if (!capConfig)
            continue;
 ...
 */
export async function runLoop(dest, options) {
    ...
    return new Promise((resolve) => {
        const intervalId = setInterval(async () => {
            ...
            try {
                ...
                console.error(`${RED}✗${RESET} Round error: ${e.message}`);
            }
            finally {
