...
const TOKEN_USAGE_RE = /\|\s*\*\*Token usage\*\*\s*\|\s*([\d,]+)\s+in\s*\/\s*([\d,]+)\s+out\s*[—-]\s*\$([\d.,]+)\s*\|/i;
...
function extractAgent(agentCell) {
    return agentCell.replace(/\s*\([^)]*\)\s*$/, '').trim();
}
function parseNumber(raw) {
    return Number(raw.replace(/,/g, ''));
}
function parseCostEntry(content, timestamp) {
...
function printSummary(entries, agentFilter) {
    ...
    const agentWidth = Math.max('Agent'.length, 'Total'.length, ...rows.map(row => row.agent.length)) + 2;
    ...
    const formatRow = (label, inputTokens, outputTokens, estimatedCost) => `  ${label.padEnd(agentWidth)}${formatInteger(inputTokens).padStart(inputWidth)}  ${formatInteger(outputTokens).padStart(outputWidth)}  ${formatCost(estimatedCost).padStart(costWidth)}`;
    console.log('💰 Squad Cost Summary');
    console.log('━━━━━━━━━━━━━━━━━━━━');
    console.log();
    console.log(`  ${'Agent'.padEnd(agentWidth)}${'Input Tokens'.padStart(inputWidth)}  ${'Output Tokens'.padStart(outputWidth)}  ${'Est. Cost'.padStart(costWidth)}`);
    console.log(divider);
...
async function getCurrentSessionCutoff(teamRoot) {
...
export async function runCost(args, teamRoot) {
