...
function printRoleRows(roles, indent = '    ') {
    if (roles.length === 0)
        return;
    const idWidth = Math.max(...roles.map(r => r.id.length), 2);
    const titleWidth = Math.max(...roles.map(r => r.title.length), 5);
    for (const role of roles) {
        console.log(`${indent}${role.emoji.padEnd(3)} ${role.id.padEnd(idWidth)}  ${role.title.padEnd(titleWidth)}  "${role.vibe}"`);
    }
}
export async function runRoles(args) {
