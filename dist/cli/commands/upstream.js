...
function readUpstreams(upstreamFile) {
    if (!storage.existsSync(upstreamFile))
        return { upstreams: [] };
    try {
...
function detectSourceType(source) {
    if (source.endsWith('.json') && storage.existsSync(path.resolve(source)))
        return 'export';
    if (source.startsWith('http://') || source.startsWith('https://') || source.startsWith('file://') || source.endsWith('.git'))
        return 'git';
    if (storage.existsSync(path.resolve(source)))
        return 'local';
            ...
            if (upstream.type === 'local' || upstream.type === 'export') {
                ...
                try {
                    if (storage.existsSync(path.join(cloneDir, '.git'))) {
                        ...
                        if (storage.existsSync(cloneDir))
                            storage.deleteDirSync(cloneDir);
